...
| `50_cobrador_desde_sesion.sql` | **PENDIENTE de aplicar** (redactado 24/08) | `registrar_pago` sella `cobrado_por`, el validador del tipo y el movimiento con el correo del JWT e ignora `p_cobrado_por`; sin correo en el JWT, rechaza el cobro. Misma firma que el 46 (sin drop). El panel ya muestra el usuario de la sesion como dato (c88cbca); el orden de despliegue es indistinto. |
| `51_limite_intentos_publicos.sql` | **PENDIENTE de aplicar** (redactado 25/08) | Rate limiting del buzon sin servicios externos: tabla `intentos_publicos` (solo la tocan las funciones; RLS sin politicas) + 10 fallos de coincidencia por IP en 15 min en `crear_solicitud` (que ya no lanza en «no coincide»: devuelve `{recibida:false, mensaje}`) y 10 notas por IP por hora en `crear_nota_solicitud`. El cliente que entiende las dos formas ya esta publicado (0b0c913); aplicar y reejecutar P-11 y P-12. |

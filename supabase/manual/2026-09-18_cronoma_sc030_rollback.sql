...
--
-- Lo que devuelve: las seis fechas `fecha_fin_plan` a como estaban. La
-- carga no toco nada mas, asi que no hay nada mas que restaurar.
--
-- OJO: esto devuelve las seis actividades a estar VENCIDAS, que es el
...
begin
  ...
  if v_n <> 6 then
    raise exception 'El respaldo tiene % filas y deberia tener 6. No se restauro nada; revise el respaldo a mano.', v_n;
  end if;
end
$guardia$;
...
select b.nombre,
 ...
 order by b.nombre;
-- Las seis filas deben salir con ok = true.

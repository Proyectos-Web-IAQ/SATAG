...
export interface CrearRegistroInput {
  ...
  // Apellidos con los que la escuela identifica a la familia. Lo llenan los
  // tipos que pertenecen a una familia de la comunidad (padres, alumno y otro
  // familiar); en maestro y administrativo viaja null.
...
export async function crearRegistro(input: CrearRegistroInput): Promise<CrearRegistroResultado> {
  ...
  const firma = await subirFirma(supabase, input.firmaDataUrl, { bucket: BUCKET_FIRMAS });

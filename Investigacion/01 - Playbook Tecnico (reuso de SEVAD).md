# Playbook Técnico — Reuso de SEVAD para SATAV
...
**GitHub Secrets requeridos** (mismos nombres para SATAV, valores nuevos):
...
Procedimiento documentado en `SEVAD/docs/tecnico/05-hosting-godaddy.md`. Para SATAV, replicar con el
subdominio nuevo (p. ej. **`satav.asuncionqro.edu.mx`**):

1. **cPanel (GoDaddy) → Subdomains:** crear `satav` bajo `asuncionqro.edu.mx`. cPanel crea la carpeta
   `public_html/satav.asuncionqro.edu.mx/`.
2. **IP de origen** ⚠️: tomar la *Shared IP Address* del cPanel (SEVAD reportó `208.109.203.174`,
   **confirmar la vigente**).
3. **Cloudflare → DNS:** registro **A**, name `satav`, apuntando a esa IP, **Proxied (nube naranja)**.
4. **Cloudflare → SSL/TLS:** modo **Full** (acepta el cert del origen), TLS mínimo 1.2.
5. **cPanel → FTP Accounts:** cuenta dedicada `deploy-satav@…` con **directorio raíz = la carpeta del
...
8. Verificar: `curl -I https://satav.asuncionqro.edu.mx/` → 200; `/admin/` → 200.
...
El **patrón** de SEVAD se reutiliza; el **esquema NO** (SEVAD son encuestas anónimas; SATAV es un
...
**Esquema NUEVO para SATAV (a diseñar en el Doc 2 — borrador):**
...
> **Diferencia clave vs SEVAD:** SATAV guarda **datos personales**. Por eso el modo autoservicio debe
...
- **Config Supabase → Auth → URL Configuration:** `Site URL` y `Redirect URLs` deben incluir el
  subdominio de SATAV y `http://localhost:3000/admin/`.
...
SEVAD organiza `docs/` así (buena plantilla para SATAV, más allá del Plan de Dirección PMBOK):
...
## 9. Qué reusar tal cual vs. qué es nuevo en SATAV
...
## 10. Checklist de clonado SEVAD → SATAV (resumen)

- [ ] Subdominio `satav.asuncionqro.edu.mx` (cPanel) + registro A proxied (Cloudflare) + cuenta FTP

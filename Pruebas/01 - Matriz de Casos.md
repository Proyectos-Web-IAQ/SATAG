...
| **F-08** | Redacción "de usted" | — | Recorrer **todo el texto de cara al usuario**: las seis pantallas del wizard (incluido el paso 5), la portada, el buzón público, la página del aviso y los mensajes de error que llegan de la base a la pantalla (los `raise exception` de los RPC se muestran tal cual) | Ningún texto tutea al usuario, venga del cliente o de la base (CC-07). Es el único caso que prueba CC-07: si se acota al wizard, el resto queda sin cobertura |
...
| **E-08** | La URL de la firma caduca | Copiar el enlace de la imagen; abrirlo **recién emitido** en una ventana sin sesión; volver a abrirlo pasado el minuto | Recién emitida **sí abre, incluso sin sesión**: la URL firmada autoriza por el token que lleva, no por la sesión de quien la abre. Lo que protege la imagen es la **caducidad corta** (60 s, el panel lo avisa en pantalla): pasado el minuto deja de servir para cualquiera. «Volver a abrirla» emite una URL nueva, distinta, que vuelve a servir |

...
| P-03 | | | *(pendiente: exige sesión real: entrar y abandonar en el reto TOTP. No se simula, porque lo que prueba es la cadena de autenticación, no las políticas)* |
...
| P-14 | | | *(vistas `security_invoker`, bloques 45 y 47)* **Primera mitad cerrada el 03-ago:** con `aal2` pero **sin** `app_metadata.rol`, `v_registros_incompletos` y `v_evidencia_firma` devuelven **0 filas las dos**. Con rol sí devuelven datos (12 y 58), así que el cero no es que las vistas estén vacías. **Falta la segunda mitad:** el mismo par de consultas con rol pero `aal` en `aal1` —con segundo factor sin completar—, que también debe dar 0 y 0. |

...
> **Estado real (26-ago-2026):** el módulo **está extraído en `lib/firma/`** (`SignaturePad` +
> `servicio` + `tipos`, con `README`), sin importar nada del dominio de SATAG: el cliente de Supabase y
> el bucket llegan por parámetro. SATAG lo consume desde `@/lib/firma`. No se implementó
> `verificarIntegridad()` del hash legal en el cliente: replicar el orden canónico de claves de `jsonb`
> en JavaScript es frágil; si hace falta, va como función SQL. Tampoco existe todavía el patrón
> genérico `AceptacionDocumento` en la base: se hará cuando exista el segundo sistema que lo use.

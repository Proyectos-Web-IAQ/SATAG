...
| `consulta` | Solo lectura | Lee padron, pagos, solicitudes, movimientos y, desde el bloque 48 (29-jul-2026), tambien `aceptaciones` y el bucket `firmas`. No ejecuta ningun RPC de escritura ni gestiona objetos del bucket: su separacion es que **no escribe**, no que vea menos. |
...
**Roles del panel (actualizado 15-jul-2026):** la fuente de verdad es exclusivamente `app_metadata.rol`, asignada por un administrador; `user_metadata` no participa en autorización. `admin` registra pagos, `ti` asigna estacionamiento e instala/actualiza/da de baja, `consulta` solo lee y `super` permite pruebas integrales controladas.

**Evidencia de firma (actualizado 29-jul-2026, bloques 47 y 48):** el panel ya muestra la firma con URL firmada temporal; el bucket sigue privado y nada se publica. La lectura de `aceptaciones` y del bucket `firmas` la tienen los **cuatro** roles del panel. El bloque 30 la habia restringido a `admin`/`super` por tratarse de PII sensible, y el 47 la amplio a `ti` para el cotejo presencial; el bloque 48 la abrio tambien a `consulta`, por decision de la Direccion de TI, para que la pantalla donde se investiga un expediente no tuviera un hueco en la auditoria. **Costo asumido de forma explicita:** la RLS de PostgreSQL es por fila, no por columna, asi que un rol que lee `aceptaciones` alcanza tambien `hash_payload`, `firma_trazos`, `ip_origen` y `user_agent`. La vista `v_evidencia_firma`, que es lo que consume el panel, no los expone; una consulta directa a la tabla si. Escribir o borrar objetos del bucket sigue siendo exclusivo de `admin`/`super`. Las RLS exigen `aal2` + rol para leer y las escrituras del expediente pasan por RPC `SECURITY DEFINER` que vuelve a validar el rol. Un usuario sin rol no lee el expediente ni entra al panel operativo. SQL y runbook: [`supabase/README.md`](../supabase/README.md).
